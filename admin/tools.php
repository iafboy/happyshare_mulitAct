...
function isLesharePriv($priv){
    if(!is_valid($priv)){
        return true;
    }
    $pres = ['activity','banner','cashreports','comment','order',
        'params','picwall','product','reports','score','statistic','supplier','user'];
    foreach($pres as $pre){
        $pos = strpos($priv,$pre);
        if($pos === 0){
            return true;
...
function getReturnMsg($success,$msg){
    if ($success) {
        return ['success' => true];
    } else {
        return ['success' => true,'errMsg'=>$msg||'操作失败'];
    }
}
