  ...
  <div class="container-fluid">
    ...
    <form id="trial-fm">
      <div class="container">
                    ...
                    <form>
                      <div>
                        ...
                        <span class="entry-group">
                          <label>活动名称：</label>
                          <span>
                            <input name="act_name" class="lfx-text" value="<?php echo $act['tp_name']; ?>" />
                          </span>
                        </span>
                        <span  class="entry-group">
                          <label>活动类型：</label>
                          <span>分享/适用报告奖励活动</span>
                        </span>
                          </div>
                          <div class="content-row">
                        <span class="entry-group">
                ...
                <td class="navi-content">
                  ...
                  <span class="entry-group" style="margin-right: 0px;">
                    ...
                    <span style="margin-left: 10px;">
                      <input name="gift_credit" class="lfx-text lfx-text-sm" type="number"
                             value="<?php echo $act['giftcredit']; ?>" />
                  ...
                  <span class="entry-group">
                    <label>
                      ...
                      <?php }else { ?>
                        <input id="is_firstorder" name="firstorder" value="1" type="checkbox" />
                      <?php }
                       ?>
        ...
        <a href="<?php echo 'index.php?route=activity/trial&token='.$token.'&link_mode=create'; ?>" class="btn lfx-btn"><?php echo "&nbsp;&nbsp;取消&nbsp;&nbsp;"; ?></a>
        </div>
        ...
        <a href="<?php echo 'index.php?route=activity/trial&token='.$token.'&link_mode=modify'; ?>" class="btn lfx-btn"><?php echo "&nbsp;&nbsp;取消&nbsp;&nbsp;"; ?></a>
        </div>
        <?php } ?>
      </div>
    </form>
...
<script type="text/javascript">
  function createAct(status){
    ...
    var url = 'index.php?route=activity/trial/create&token=<?php echo $token; ?>';
  ...
  function saveAct(status){
    ...
    params.act_status = status;
    params.act_id = '<?php echo $act["tp_id"]; ?>';
    var url = 'index.php?route=activity/trial/modify&token=<?php echo $token; ?>';
    $.model.commonAjax(url,params,function(data){
      if(data.success){
        window.location = data.location;
      }else{
  ...
  function changeMainImage(image){
    image = $(image);
    var upload_url = 'index.php?route=common/temp/fileuploader&token=<?php echo $token; ?>&directory=temp/activity/trial/main';
    var old_image = $(image).parent().find('input').val();
    commonFileUpload(upload_url,
            {file_name:'act_<?php echo $act["tp_id"]; ?>_main_temp',resize_width:200,resize_height:200,delete_path:old_image}
            ,function(data){
  ...
  function uploadMainImage(image){
    ...
    var upload_url = 'index.php?route=common/temp/fileuploader&token=<?php echo $token; ?>&directory=temp/activity/trial/main';
    commonFileUpload(upload_url,
            {file_name:'act_<?php echo $act["tp_id"]; ?>_main_temp',resize_width:200,resize_height:200}
            ,function(data){
              if(data.success===true){
                var path = data['file_path'];
                var image_path = data['image_path'];
