...
class ModelUserUser extends MyModel {
	public function addUser($data) {
        $sql = "INSERT INTO `" . DB_PREFIX
            ...
            . "', date_added = NOW() , phone = '".$this->db->escape($data['phone'])."' ";
		$this->db->query($sql);
	}

	public function editUser($user_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "user` SET username = '" . $this->db->escape($data['username']) . "', user_group_id = '" . (int)$data['user_group_id'] . "', fullname = '" . $this->db->escape($data['fullname']) . "', email = '" . $this->db->escape($data['email']) . "', image = '" . $this->db->escape($data['image']) . "', status = '" . (int)$data['status'] . "' , phone = '".$data['phone']."' WHERE user_id = '" . (int)$user_id . "'");
	...
	public function getUsers($data = array()) {
		$sql = "SELECT a.*,b.name as user_group FROM `" . DB_PREFIX . "user` a , ".getTable('user_group').' b where a.user_group_id = b.user_group_id ';
