...
class ModelOrderList extends MyModel {
    public function querySuppliers($orderId){
        $sql='select DISTINCT(b.supplier_name) as sname from '.getTable('order_product').' a ,'.getTable('supplier').' b where a.order_id='.$orderId.' and a.supplier_id=b.supplier_id ';
        $rows=parent::queryRows($sql);
        //echo $sql;
        $res='';
        if(!is_null($rows)&&isset($rows)){
            foreach($rows as $c){
                $res.= $c['sname'].' ';
            }
        }
