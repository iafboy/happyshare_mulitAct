...
class ModelReportSale extends Model {
	...
	public function getTotalLfxSales($data = array()) {
		$sql = "SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id <> '1'";
	...
	public function getTotalShipping($data = array()) {
		...
		if (!empty($data['filter_order_status_id'])) {
			$sql .= " AND order_status = '" . (int)$data['filter_order_status_id'] . "'";
		} else {
			$sql .= " AND order_status in( '0','2','3','4','10','11','12')";
		}
