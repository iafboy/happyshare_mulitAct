...
class ControllerUserUser extends Controller {
	...
	public function delete() {
        ...
        if(!is_valid($user_id)){
            writeJson(['success'=>false,'errMsg'=>'用户ID无效']);
            return;
        }
        $this->model_user_user->deleteUser($user_id);
        $location = $this->url->link('user/user', 'token=' . $this->session->data['token'] , 'SSL');
        writeJson(['success'=>true,location=>html_entity_decode($location)]);
	}
 ...
 */
    ...
    if (!isset($this->request->post['phone']) || empty($this->request->post['phone'])) {
		...
		if (!is_valid($this->request->post['phone'])) {
			$this->error['warning'] = '手机号不符合要求';
		    return !$this->error;
		}

    $phone_num = $this->request->post['phone'];
