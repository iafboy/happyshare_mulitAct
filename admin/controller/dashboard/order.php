...
class ControllerDashboardOrder extends Controller {
	...
	*/
	public function index() {
		...
		// Total Orders
		$this->load->model('order/list');
		$this->load->model('common/order');

		$today = $this->model_order_list->getTotalOrders(array('filter_date_added' => date('Y-m-d', strtotime('-1 day'))));
		$yesterday = $this->model_order_list->getTotalOrders(array('filter_date_added' => date('Y-m-d', strtotime('-2 day'))));
		$difference = $today - $yesterday;
		...
		$order_total = $this->model_common_order->queryOrderTotalCount();
		...
		$data['order'] = $this->url->link($this->module_name, 'token=' . $this->session->data['token'], 'SSL');
