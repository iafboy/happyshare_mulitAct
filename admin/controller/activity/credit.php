<?php
class ControllerActivityCredit extends MyController {
    ...
    private $module_name = 'activity/credit';
    ...
    public function create(){
        ...
        $img_path = 'activity/credit/main/'.str_replace('_temp','',$img_name);
        ...
        $success = $this->model_activity_credit->createCredit($post);
        if($success===true){
    ...
    public function modify(){
        ...
        $success = $this->model_activity_credit->modifyCredit($post);
        if($success===true){
            writeJson(['success'=>true,'location'=> html_entity_decode(
                $this->url->link('activity/list', 'token=' . $this->session->data['token'])
            )]);
    ...
    protected function getCredit($lans) {
        ...
        if($link_mode=='create'){
            $credit = ['cp_id' => $this->model_activity_credit->queryNextCreditId()];
        }else if($link_mode=='modify'){
            $sub_id = $this->request->get['sub_id'];
            $credit = $this->model_activity_credit->queryCreditById($sub_id);
        }
