...
class ControllerCommonFileManager extends Controller {
	...
	public function upload() {
		...
		if (isset($this->request->get['directory'])) {
			//$directory = rtrim(DIR_IMAGE . 'catalog/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
			$directory = rtrim(DIR_IMAGE . 'supplier/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
			$file_url = rtrim('supplier/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
			$directory = DIR_IMAGE . 'supplier/';
			$file_url = 'supplier/';
//			$directory = DIR_IMAGE . 'catalog';
		}
    ...
    if (!$json) {
      ...
      //liuhang add : change file name, add timestamp
      $suffix = substr(strrchr($filename, '.'), 1);
      $purename = basename($filename,".".$suffix);
      $time = time();
      $purename = $purename . '_' . $time;
      $filename = $purename . '.' . $suffix;
