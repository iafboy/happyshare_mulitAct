...
class ControllerCatalogReview extends Controller
{
    ...
    public function reply()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->model_catalog_review->replyReview($this->request->get['review_id'], $this->request->post, $this->session->data['supplier_id']);
    ...
    protected function getList()
    {
        ...
        $categories = $this->model_catalog_review->getCategories();
        $product_types = $this->model_catalog_review->getProductTypes();
        ...
        foreach ($results as $result) {
            $count=0;
            $message='';
            if(!is_null($result['reply'])&&isset($result['reply'])){
                $count=1;
                $jsonmessage=json_decode($result['reply']);
            ...
            $data['reviews'][] = array(
                ...
                'product_id' => $result['pro_id'],
                'name' => $result['name'],
                'cid' => $result['customer_id'],
                'cname' => $result['fullname'], //customer fullname
                'content' => $result['text'],
                'reply_count' => $count,
                'reply_msg' => urldecode($message),
                //'status'   => ($result['status']) ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
                'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
                'edit' => $this->url->link('catalog/review/ReplyOrAddnew', 'token=' . $this->session->data['token'] . '&review_id=' . $result['review_id'] . '&op_type=1', 'SSL'),
                'reply' => $this->url->link('catalog/review/ReplyOrAddnew', 'token=' . $this->session->data['token'] . '&review_id=' . $result['review_id'] . '&op_type=0', 'SSL')
            );
        }
        ...
        $data['sort_pid'] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=p.product_id' . $url, 'SSL');
        $data['sort_pname'] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
        $data['sort_sid'] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=p.supplier_id' . $url, 'SSL');
    ...
    protected function getForm()
    {
        ...
        if (isset($this->request->get['op_type']) && ($this->request->get['op_type'] == 0)) {
            $data['action'] = $this->url->link('catalog/review/reply', 'token=' . $this->session->data['token'] . '&review_id=' . $this->request->get['review_id'], 'SSL');
        } else {
            $data['action'] = $this->url->link('catalog/review/add', 'token=' . $this->session->data['token'], 'SSL');
        }
