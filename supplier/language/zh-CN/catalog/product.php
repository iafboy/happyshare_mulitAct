<?php

// Heading
$_['heading_title']          = '商品管理';
$_['heading_title_edit']     = '商品编辑';
$_['heading_title_add']      = '商品发布';

// Text
$_['text_success']           = '成功: 已经修改商品！';
$_['text_list']              = '商品列表';
$_['text_add']               = '添加商品';
$_['text_edit']              = '编辑商品';
$_['text_plus']              = '+';
$_['text_minus']             = '-';
$_['text_default']           = '默认';
$_['text_option']            = '选项';
$_['text_option_value']      = '选项值';
$_['text_percent']           = '百分比';
$_['text_amount']            = '固定值';

$_['text_status_0']            = '待审批';
$_['text_status_1']            = '审批未通过';
$_['text_status_2']            = '审批通过 未上架';
$_['text_status_3']            = '已上架';
$_['text_status_4']            = '下架';
$_['text_status_5']            = '出库';
$_['text_status_wrong']        = '错误状态，请通知管理员！';

$_['text_product_edit_basicinfo']   = '商<br>品<br>基<br>本<br>信<br>息<br>';
$_['text_product_edit_document']    = '商<br>品<br>文<br>案<br>图<br>片<br>';
$_['text_product_edit_shareinfo']   = '分<br>享<br>文<br>案<br>（<br>可<br>选<br>）<br>';


// Column
$_['column_name']            = '商品名称';
$_['column_model']           = '商品类型';//'型号';
$_['column_image']           = '图像';
$_['column_price']           = '市场价';//'价格';
$_['column_quantity']        = '库存量';//'数量';
$_['column_status']          = '状态';
$_['column_action']          = '编辑商品';//'操作';
/*liuhang add for product management*/
$_['column_id']              = '商品编号';
$_['column_origin']          = '来源地';
$_['column_sales']           = '商品累计销量';
$_['column_supply_price']    = '供货价';
$_['column_document']        = '分享文案数量';
$_['column_comments']        = '评论数量';
$_['column_product_recommend_index']        = '推荐指数';

// Entry
$_['entry_name']             = '商品名称';
$_['entry_description']      = '描述';
$_['entry_meta_title'] 	     = 'Meta Tag 标题';
$_['entry_meta_keyword'] 	 = 'Meta Tag 关键词';
$_['entry_meta_description'] = 'Meta Tag 描述';
$_['entry_keyword']          = 'SEO Keyword';
$_['entry_model']            = '商品类型';//'型号';
$_['entry_sku']              = 'SKU';
$_['entry_upc']              = 'UPC';
$_['entry_ean']              = 'EAN';
$_['entry_jan']              = 'JAN';
$_['entry_isbn']             = 'ISBN';
$_['entry_mpn']              = 'MPN';
$_['entry_location']         = '生产地';
$_['entry_shipping']         = '需要配送';
$_['entry_manufacturer']     = '制造商/品牌';
$_['entry_store']            = '商店';
$_['entry_date_available']   = '上架日期';
$_['entry_quantity']         = '数量';
$_['entry_minimum']          = '最小购买数量';
$_['entry_stock_status']     = '缺货时状态';
$_['entry_price']            = '价格';
$_['entry_tax_class']        = '税率';
$_['entry_points']           = 'Points';
$_['entry_option_points']    = 'Points';
$_['entry_subtract']         = '减少库存';
$_['entry_weight_class']     = '重量单位';
$_['entry_weight']           = '重量';
$_['entry_dimension']        = '尺寸 (长 x 宽 x 高)';
$_['entry_length_class']     = '尺寸单位';
$_['entry_length']           = '长';
$_['entry_width']            = '宽';
$_['entry_height']           = '高';
$_['entry_image']            = '图像';
$_['entry_customer_group']   = '会员等级';
$_['entry_date_start']       = '开始日期';
$_['entry_date_end']         = '结束日期';
$_['entry_priority']         = '优先级';
$_['entry_attribute']        = '属性';
$_['entry_attribute_group']  = '属性分组';
$_['entry_text']             = '内容';
$_['entry_option']           = '选项';
$_['entry_option_value']     = '选项值';
$_['entry_required']         = '必填项';
$_['entry_status']           = '状态';
$_['entry_sort_order']       = '排序';
$_['entry_category']         = '分类';
$_['entry_filter']           = '筛选';
$_['entry_download']         = '下载文件';
$_['entry_related']          = '相关商品';
$_['entry_tag']          	 = '商品标签';
$_['entry_reward']           = '奖励积分';
$_['entry_layout']           = '布局覆盖';
$_['entry_recurring']        = '分期付款';


//liuhang add for product management
$_['entry_product_code']               = '商品编号';
$_['entry_product_name']               = '商品名称（模糊查询）';
$_['entry_product_place']              = '商品来源地';
$_['entry_product_place_shipment']     = '商品发货地';
$_['entry_product_catagory']           = '商品类型';
$_['entry_product_stock']              = '库存';
$_['entry_product_price_market']       = '市场价';
$_['entry_product_price_supplier']     = '供货价金额';
$_['entry_product_price_store']        = '供货价';
$_['entry_product_status']             = '状态';
$_['entry_product_sales']              = '商品销量';
$_['entry_product_comments']           = '评论数量';
$_['entry_product_weight']             = '商品物流计算重量';
$_['entry_product_volume']             = '商品物流计算体积';
$_['entry_product_tax']                = '商品代收税金额';
$_['entry_charge_mode']                = '计费方式';
$_['entry_product_return']             = '退货设置';
$_['entry_product_recommend_index']    = '推荐指数';
$_['entry_reward']                     = '回馈积分';
$_['text_product_return_no']            = '不允许退货';
$_['text_product_return_yes1']          = '签收';
$_['text_product_return_yes2']          = '天内允许退货';
$_['text_provinces']                   = array('北京市','天津市','上海市','重庆市','河北省',
                                               '河南省','云南省','辽宁省','黑龙江省','湖南省',
                                               '安徽省','山东省','新疆维吾尔','江苏省','浙江省',
                                               '江西省','湖北省','广西壮族','甘肃省','山西省',
                                               '内蒙古','陕西省','吉林省','福建省','贵州省',
                                               '广东省','青海省','西藏','四川省','宁夏回族',
                                               '海南省','台湾省','香港特别行政区','澳门特别行政区'
                                               );

// Help
$_['help_keyword']           = '不要用空格，使用-链接关键字，确保该关键词为全站唯一。';
$_['help_sku']               = 'Stock Keeping Unit';
$_['help_upc']               = 'Universal Product Code';
$_['help_ean']               = 'European Article Number';
$_['help_jan']               = 'Japanese Article Number';
$_['help_isbn']              = 'International Standard Book Number';
$_['help_mpn']               = 'Manufacturer Part Number';
$_['help_manufacturer']      = '(输入时自动筛选结果)';
$_['help_minimum']           = '加入订单时所需最小数量';
$_['help_stock_status']      = '缺少该商品时所显示的数量';
$_['help_points']            = '购买此商品所需奖励积分。如果您不想让客户使用奖励积分来购买此产品，请将值设为0。';
$_['help_category']          = '(输入时自动筛选结果)';
$_['help_filter']            = '(输入时自动筛选结果)';
$_['help_download']          = '(输入时自动筛选结果)';
$_['help_related']           = '(输入时自动筛选结果)';
$_['help_tag']          	 = '英文逗号分割';

// Error
$_['error_warning']          = '警告: 存在错误，请检查！';
$_['error_permission']       = '警告: 无权限修改商品！';
$_['error_name']             = '商品名称必须介于3-255字符之间！';
$_['error_meta_title']       = 'Meta 标题必须介于3-255字符之间！';
$_['error_model']            = '商品型号必须介于1-64字符之间！';
$_['error_keyword']          = '该 SEO keyword 已经存在了！';

$_['button_query']  =  '查询';
