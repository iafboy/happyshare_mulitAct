...
class ModelProductEdit extends MyModel {
    ...
    public function queryMainTitleImage($product_id,$index){
        $sql = 'select img_'.$index.' image from '.getTable('product').' where product_id = '.to_db_int($product_id);
        $p = parent::querySingleRow($sql);
        if(isset($p) && !empty($p)){
            return $p['image'];
        }
    ...
    public function saveProduct($data=array()){
        ...
        if(is_valid($data['express_template'])){
            $sql .= ' express_template = '.to_db_int($data['express_template']).' ,';
        }
