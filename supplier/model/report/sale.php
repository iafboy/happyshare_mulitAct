...
class ModelReportSale extends Model {
	...
	public function getTotalSales($data = array()) {
		$sql = "SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status > '0'";
	...
	public function getTotalSupplierSales($data = array(),$supplierid) {
		$sql = "SELECT SUM(p.total) AS total FROM `" . DB_PREFIX . "order` p,`".DB_PREFIX."order_product` o where p.order_id=o.order_id and o.supplier_id='".$supplierid."' and order_status = '10'";
	...
	public function getTotalOrdersByCountry() {
		$query = $this->db->query("SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "country` c ON (o.payment_country_id = c.country_id) WHERE o.order_status = '10' GROUP BY o.payment_country_id");

		return $query->rows;
	}
		
	// Orders
	public function getTotalOrdersByDay($supplierid) {
		$implode = array();
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(p.date_added) AS hour FROM `" . DB_PREFIX . "order` p,`" . DB_PREFIX . "order_product` o WHERE p.order_id=o.order_id and o.supplier_id='".$supplierid."' and o.supplier_id='".$supplierid."' and p.order_status_id IN(" . implode(",", $implode) . ") AND DATE(p.date_added) = DATE(NOW()) GROUP BY HOUR(p.date_added) ORDER BY p.date_added ASC");
	...
	public function getTotalOrdersByWeek($supplierid) {
		$implode = array();
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` p,`" . DB_PREFIX . "order_product` o WHERE p.order_id=o.order_id and o.supplier_id='".$supplierid."' and  o.supplier_id='".$supplierid."' and order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') GROUP BY DAYNAME(date_added)");
	...
	public function getTotalOrdersByMonth($supplierid) {
		$implode = array();
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` p,`" . DB_PREFIX . "order_product` o WHERE p.order_id=o.order_id and o.supplier_id='".$supplierid."' and  order_status IN(" . implode(",", $implode) . ") AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(date_added)");
	...
	public function getTotalOrdersByYear($supplierid) {
		$implode = array();
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` p,`" . DB_PREFIX . "order_product` o WHERE p.order_id=o.order_id and o.supplier_id='".$supplierid."' and order_status_id IN(" . implode(",", $implode) . ") AND YEAR(date_added) = YEAR(NOW()) GROUP BY MONTH(date_added)");
	...
	public function getTaxes($data = array()) {
		...
		if (!empty($data['filter_order_status_id'])) {
			$sql .= " AND o.order_status = '" . (int)$data['filter_order_status_id'] . "'";
		} else {
