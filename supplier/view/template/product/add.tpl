  ...
  <div class="container-fluid">
    ...
    <div class="container">
                ...
                <div>
                  <!--<form id="product-edit-fm">-->
                    ...
                    <div class="content-row">
                      <span class="entry-group">
                        <label>商品库存：</label>
                        <span><input name="quantity" class="lfx-text lfx-text-sm" value="<?php echo $product['quantity']; ?>"/></span>
                      </span>
                      <span class="entry-group">
                        <label>商品重量：</label>
                        <span><input name="weight" class="lfx-text lfx-text-sm" value="<?php echo $product['weight']; ?>"/>Kg</span>
                      </span>
                      <span class="entry-group">
                        <label>商品体积：</label>
                        <span><input name="volume" class="lfx-text lfx-text-sm" value="<?php echo $product['volume']; ?>"/>m<span>3</span></span>
                      </span>
                     <!--
                      <span class="entry-group">
                        <label>计费方式：</label>
                        <span>
                          <select class="lfx-select lfx-text-sm" name="charge_type" >
                            <?php
                              foreach($chargetypes as $charge){
                                if($product['charge_type'] == ($charge['id'].'')){
                                  echo '<option selected value="'.$charge['id'].'">'.$charge['name'].'</option>';
                                }else{
                    ...
                    <div class="content-row">
                      <span  class="entry-group">
                        <label>市场价：</label>
                        <span><input type="number" name="market_price" value="<?php echo $product['market_price']; ?>" class="lfx-text" style="width: 100px;margin-right: 10px;" />元</span>
                      </span>
                      <span  class="entry-group">
                        <label>返利：</label>
                        <span><input type="number" name="interest_price" value="<?php echo $product['interest_price']; ?>" class="lfx-text" style="width: 100px;margin-right: 10px;" />元</span>
                      </span>
                      <span  class="entry-group">
                        <label>平台价：</label>
                      ...
                      <span  class="entry-group">
                        <label>推荐指数：</label>
                        <span><input type="number" name="shareLevel" value="<?php echo $product['shareLevel']; ?>" class="lfx-text" style="width: 100px;margin-right: 10px;" />(100最大)</span>
                      </span>
                    </div>
                  <?php if ($self_set != 0) { ?>
                    <div class="content-row">
                    ...
                    <div class="content-row">
                      <span class="entry-group">
                        ...
                        <span>
                          ...
                          <input type="radio" name="can_return" value="0" checked />不允许退货
                          <input type="radio" name="can_return" value="1" />
                          <input class="lfx-text-xs lfx-text" type="number" name="return_limit" value="" />天内允许退货
                          <?php } else { ?>
                          <input type="radio" name="can_return" value="0" />不允许退货
                          <input type="radio" name="can_return" value="1" checked />
                          <input class="lfx-text-xs lfx-text" type="number" name="return_limit" value="<?php echo $product['return_limit']; ?>" />天内允许退货
                          <?php } ?>
                        </span>
                      </span>
                    </div>
                                  ...
                                  <?php for($x = 1; $x < 2; $x ++){
                                    if(isset($share_case['imgurl'.$x]) && is_valid($share_case['imgurl'.$x])){ ?>
                                      <div class="image-container-sm sharecase-image" data-case-img-index="<?php echo $x; ?>">
                                        ...
                                        <a onclick="onclick="doDelSharecaseImage(this)" class="del-btn on fa fa-fw fa-trash"></a>
                                      </div>
                                    <?php }else{ ?>
                                      <div class="image-container-sm add-case-row">
                                        <a onclick="return false;" class="img-thumbnail" style="padding: 0px;cursor: pointer;width:150px;height:150px;">
                                          <img width="100%" height="100%" src="<?php echo $to_add_img; ?>"
                                               data-placeholder="<?php echo $placeholder; ?>"
      ...
      <div class="row" style="text-align: center;margin-top: 20px;">
        <button type="button" class="btn lfx-btn" onclick="addProduct(0)">提交审批</button>
...
<script type="text/javascript">
  ...
  function addProduct(status){
    var url = 'index.php?route=product/add/addProduct&token=<?php echo $token; ?>';
...
$.model.commonAjax(url,params, function (data) {
  ...
  function getParams(){
    ...
    //if(params.interest_price >  params.store_price){
...
//    }
    ...
    if(!is_valid_str(params.main_image)){
      showErrorText('主图缺失！');
      return false;
    }
    $suc = true;
    var subs_str = '';
    $('.sub-images-box .sub-image').each(function() {
      var that = this;
      var seq = $(that).data('sub-index');
      subs_str = subs_str + seq +',';
      if(!is_valid_str($(that).find('input').val())){
        $suc = false;
      }
    });
    if(!is_valid_str(subs_str)){
      showErrorText('文案图片错误！');
      return false;
    }else{
      subs_str = subs_str.substr(0,subs_str.length - 1);
      params.subs = subs_str;
    }
    if(!$suc){
      showErrorText('文案图片上传错误！');
      return false;
    }
    ...
    $cases.each(function(i){
      ...
      if(!is_valid_str($(this).find('.sharecase-image input').val())){
        case_ok = false;
      }
    });

    if(case_ok===false){
      showErrorText('分享文案参数缺失');
      return false;
    }
  ...
  function appendShareCase(){
    var case_index;
    if($('.product-sharecase-box tr.case-row').length == 0){
      case_index = 1;
    }else{
      case_index = $('.product-sharecase-box tr.case-row').last().data('sharecase-seq') + 1;
    }
  ...
  function replaceShareCaseImage(image,case_index,img_index){
    var upload_url = 'index.php?route=product/add/uploadImage&token=<?php echo $token; ?>';
    commonFileUpload(upload_url,
      {type:3,product_no:'<?php echo $product["product_no"]; ?>',image_index:img_index,case_index:case_index},
      function(data){
      if(data.success===true){
        ...
        <input type="hidden" name="share_image_'+case_index+'_'+img_index+'" value="'+image_path+'" />';
  ...
  function appendShareCaseImage(image,case_index){
    ...
    commonFileUpload(upload_url,
      ...
      if(data.success===true){
        ...
        var image_path = data['image'];
        var html =
        '<div class="image-container-sm sharecase-image" data-case-img-index="'+img_index+'"> \
        <a onclick="return false;" class="img-thumbnail" style="padding: 0px;cursor: pointer;width:150px;height:150px;"> \
        <img width="100%" height="100%" src="'+path+'?t='+new Date().getTime()+'" \
  ...
  function doDelSharecaseImage(obj){
    ...
    var index = $(image).parents('.sharecase-image').data('case-img-index');
    var imag_path =$(image).parents('.sharecase-image').find('input').val();
    var win =  confirmWideHtmlWin('提示','确认删除么？', function (data) {
      var url = 'index.php?route=product/add/deleteSharecaseImage&token=<?php echo $token; ?>';
      $.model.commonAjax(url,{image_path:imag_path}, function (data) {
        if(data.success === true){
  ...
  function doDelSubImage(obj){
    ...
    var imag_path =$(image).parents('.sub-image').find('input').val();
    var win =  confirmWideHtmlWin('提示','确认删除么？', function (data) {
      var url = 'index.php?route=product/add/deleteSubImage&token=<?php echo $token; ?>';
      $.model.commonAjax(url,{image_path:imag_path}, function (data) {
        if(data.success === true){
  ...
  function doDealShareCase(obj,seq){
    ...
    var image_path = $row.find('.sharecase-image-box input').val();
    var win =  confirmWideHtmlWin('提示','确认删除么？', function (data) {
      var url = 'index.php?route=product/add/deleteShareCase&token=<?php echo $token; ?>';
      $.model.commonAjax(url,{image_path:image_path}, function (data) {
        if(data.success === true){
          $row.remove();
          win.close();
        }else{
  ...
  function uploadMainImage(image){
    ...
    var upload_url = 'index.php?route=product/add/uploadImage&token=<?php echo $token; ?>';
    commonFileUpload(upload_url,
      {type:1,product_no:'<?php echo $product["product_no"]; ?>'},
      function(data){
      if(data.success===true){
          var path = data['file_path'];
          var image_path = data['image'];
  ...
  function uploadMainImage1(image){
    ...
    var upload_url = 'index.php?route=product/add/uploadImage&token=<?php echo $token; ?>';
    commonFileUpload(upload_url,
            {type:4,product_no:'<?php echo $product["product_no"]; ?>'},
            function(data){
              if(data.success===true){
                var path = data['file_path'];
                var image_path = data['image'];
  ...
  function uploadMainImage2(image){
    ...
    var upload_url = 'index.php?route=product/add/uploadImage&token=<?php echo $token; ?>';
    commonFileUpload(upload_url,
            {type:5,product_no:'<?php echo $product["product_no"]; ?>'},
            function(data){
              if(data.success===true){
                var path = data['file_path'];
                var image_path = data['image'];
  ...
  function uploadMainImage3(image){
    ...
    var upload_url = 'index.php?route=product/add/uploadImage&token=<?php echo $token; ?>';
    commonFileUpload(upload_url,
            {type:6,product_no:'<?php echo $product["product_no"]; ?>'},
            function(data){
              if(data.success===true){
                var path = data['file_path'];
                var image_path = data['image'];
  ...
  function changeSubImage(image){
    ...
    var index = $(image).parents('.sub-image').data('sub-index');
    commonFileUpload(upload_url,
      {product_no:'<?php echo $product["product_no"]; ?>',sub_index:index,type:2}
      ,function(data){
        if(data.success===true){
          var path = data['file_path'];
          var image_path = data['image'];
          ...
          data-placeholder="<?php echo $placeholder; ?>" onclick="changeSubImage(this)" />'
                  +'<input type="hidden" name="sub_image_'+index+'" value="'+image_path+'" />';
          $(image).parent().empty().append(html);
        }else{
          showErrorText(data.error);
        }
