...
$(function () {
	...
	function errTipsBig(text){
		$('body').append('<div class="tipstext_big">'+text+'</div>');
		setTimeout(function(){
		$('tipstext_big').fadeOut(function(){
		$('.tipstext_big').remove();
		});
	},1500);
	}
window.errTipsBig = errTipsBig;
    //公共菜单链接跳转
    ...
    /*setTimeout(function(){
        $('img.ui-img').each(function(){
            var _this = $(this);
                // expected scale
                var scale = _this.attr('data-width') / _this.attr('data-height');
                var image = new Image();
                image.onload = function(){
                    var real_scale = image.width/image.height;
                    // wider than expected
                    if(real_scale >= scale){
                        _this.css({
...
function removeFromMyCollect(productId,sucCb,failCb){
    var removeMcURL = baseUrl + 'newwap/APIs/product/removeCollection.php';
    detectLoginAndDeal(function (customerId) {
        $.get(removeMcURL,{customerId:customerId,productId:productId},function(data){
...
function parseFormatNum(numStr,decimal,tf){
    ...
    if(tf !== false){
        if(dealStr.indexOf('.') != -1){
            ...
            // 9.90
            var _dep = -1;
            for(var i = dealStr.length-1;i>=pos;i--){
            ...
            if(_dep != -1){
                dealStr = dealStr.substring(0,_dep);
                if(dealStr.indexOf('.') ==  dealStr.length-1){
                    dealStr = dealStr.substr(0,dealStr.length-1);
                }
            }
        }
    }
...
function getNumberWithDecimal(numStr,decimal){
    ...
    if(index== -1){
        ...
        if(decimalLen > decimal){
            numStr = numStr.substr(0,length-(decimalLen-decimal));
